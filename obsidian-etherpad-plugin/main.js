    ...
    (function() {
      ...
      exports.connect = function(options) {
        ...
        api.call = function(functionName2, functionArgs, callback) {
          ...
          if (functionName2 === 'setText') {
            apiOptions = _.extend({
              "apikey": this.options.apikey,
            }, {"padID":functionArgs.padID},);
            getpost='POST';
            httpOptions = _.extend(this.options, {
              path: rootPath + functionName2 + "?" + querystring.stringify(apiOptions),
              method: 'POST',
          ...
          if(functionName2==='setText'){
            let data = `text=${encodeURIComponent(functionArgs.text)}`;
...
var DEFAULT_SETTINGS = {
  ...
  apikey: "",
  random_pad_id: true,
  dbFileName: '.obsidian/plugins/obsidian-etherpad-plugin/etherpad-ids.json',
...
var Etherpad = class extends import_obsidian.Plugin {
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      try {
        ...
        this.loadedDB = true;
        console.log(`Etherpad: db loaded`);
      }
      catch (e) {
          console.error(`Etherpad plugin cant read db: ${e}`);
          this.db = {};
      }
      this.registerEvent(this.app.workspace.on("file-open", (note) => __async(this, null, function* () {
        if (this.loadedDB) {
          console.log('Etherpad: check pulling');
          this.auto_replace_note_from_etherpad(note);  
        }
      })));
      this.registerEvent(this.app.vault.on("modify", (note) => __async(this, null, function* () {
        if (!this.loadedDB)
        return;
        if (!note.name)
          return;
        let etherpad_info = this.get_info(note);
        if (etherpad_info!=null && etherpad_info.etherpad_id!=null) {
          console.log('检测到输入');
          if (this.noteTimers[note] == null) {
            console.log('已设置倒计时');
            this.noteTimers[note] = true;
            this.registerInterval(window.setTimeout(()=>this.ready_to_push(note, etherpad_info), 10000));
          }
        }
      })));
      this.addCommand({
        id: "etherpad-push-pad-force",
        name: "Force Push Obsidian ==> Etherpad",
        editorCallback: (editor, view) => __async(this, null, function* () {
          ...
          let etherpad_info = this.get_info(note);
          if (etherpad_info) {
            this.push_note_to_etherpad(etherpad_info.etherpad_id, note_text_without_frontmatter, note);
            return;
          }
          let pad_id = this.settings.random_pad_id ? makeid(12) : note.basename;
          this.create_note_on_etherpad(pad_id, note_text_without_frontmatter, note);
        })
      });
      this.addCommand({
        id: "etherpad-push-pad-safe",
        name: "Safe Push Obsidian ==> Etherpad",
        editorCallback: (editor, view) => __async(this, null, function* () {
          const note = view.file;
          if (!note.name)
            return;
          ...
          let etherpad_info = this.get_info(note);
          if (etherpad_info) {
            this.auto_push_note_to_etherpad(etherpad_info.etherpad_id, note_text_without_frontmatter, note);
            return;
          }
          let pad_id = this.settings.random_pad_id ? makeid(12) : note.basename;
          console.log(pad_id);
      ...
      this.addCommand({
        id: "etherpad-pull-pad-force",
        name: "Force Pull Etherpad ==> Obsidian",
        editorCallback: (editor, view) => __async(this, null, function* () {
          const note = view.file;
      ...
      this.addCommand({
        id: "etherpad-pull-pad-safe",
        name: "Safe Pull Etherpad ==> Obsidian",
        editorCallback: (editor, view) => __async(this, null, function* () {
          const note = view.file;
          this.auto_replace_note_from_etherpad(note);
        })
      ...
      this.addCommand({
        ...
        editorCallback: (editor, view) => __async(this, null, function* () {
          ...
          let frontmatter = this.get_frontmatter(note);
          let etherpad_info = this.get_info(note);
          if (etherpad_info && etherpad_info.etherpad_id) {
            let url = this.get_url_for_pad_id(etherpad_info.etherpad_id);
            console.log(url);
            window.open(url);
          }
        })
      ...
      this.addCommand({
        id: "etherpad-remove",
        name: "解除 etherpad id 绑定",
        editorCallback: (editor, view) => __async(this, null, function* () {
          let note = view.file;
          if (!note.name)
            return;
          this.remove_from_etherpad(note);
        })
      });
  ...
  ready_to_push(note, etherpad_info) {
    return __async(this, null, function* () {
      console.log('Etherpad: try pushing');
      let note_text = yield this.app.vault.read(note);
      let note_text_without_frontmatter = yield this.get_text_without_frontmatter(note_text, note);
      this.auto_push_note_to_etherpad(etherpad_info.etherpad_id, note_text_without_frontmatter, note);
      delete this.noteTimers[note];
    });
  }
  loadSettings() {
    return __async(this, null, function* () {
...
`;
  ...
  update_db(note, id) {
    ...
    this.update_db(note,id,time);
  }
  update_db(note, id, time) {
    if (!note)
    return;
    var fileName = note.path;
    this.db[fileName]={'etherpad_id': id, "etherpad_last_sync": time};
    console.log(`Etherpad: db updated ${id},${time} `);
    this.writeDb(this.db);
  }
  remove_from_etherpad(note) {
    if (!note)
    return;
    var fileName = note.path;
    new import_obsidian.Notice(`${note.name}已从远端解除绑定`);
  ...
  get_url_for_pad_id(pad_id) {
    ...
    if (!this.settings.port) {
      return `http://${this.settings.host}/p/${pad_id}`;
    }
  ...
  replace_note_from_etherpad(note) {
    return __async(this, null, function* () {
      ...
      if (!etherpad_info)
        return;
      this.etherpad.getHTML({ padID: etherpad_info.etherpad_id }, (err, data) => {
        if (err) {
          console.log("err", err);
          ...
          let content = (0, import_obsidian2.stringifyYaml)(frontmatter);
          var frontmatter_text;
          if (content=='{}\n') {
            frontmatter_text='';
          } else {
            let frontmatter_text = `---
...
console.log(`frontmatter-test:${frontmatter_text}`);
          let note_html = data.html;
          let note_text = td.turndown(note_html);
          console.log('下一次 last sync 时间戳是本地下载远端内容的时间戳');
          this.app.vault.modify(note, frontmatter_text + note_text.replace(/[^\S\r\n]+$/gm, "")).then(()=>this.update_db(note, etherpad_info.etherpad_id));
          new import_obsidian.Notice(`${etherpad_info.etherpad_id}已从远端拉取最新内容到本地`);
          }
      });
    });
  }
  auto_replace_note_from_etherpad(note) {
    return __async(this, null, function* () {
      if (note == null)
        return;
      let etherpad_info = this.get_info(note);
      ...
      this.show_logo();
      let localEdit = note.stat.mtime;
      let lastSync = etherpad_info.etherpad_last_sync;
      console.log(`localEdit:${localEdit}, lastSync:${lastSync}`);
      if (!localEdit || localEdit <= lastSync) {
        this.replace_note_from_etherpad(note);
      } else {
        new import_obsidian.Notice(`本地有新修改，未能同步。请手动同步内容！`);
      }
    });
  }
  show_logo(){
    ...
    this.statusBarItemEl.createEl("div", {cls:'etherpad_icon'});
    this.statusBarItemEl.createEl("span", {text:'📗'});
    } 
      this.statusBarItemEl.toggleClass("etherpad_icon", false);
  }
  hide_logo(){
    if (this.statusBarItemEl != null) {
      this.statusBarItemEl.toggleClass("etherpad_icon", true);
    }
  }
  create_note_on_etherpad(id, text, note) {
    this.etherpad.createPad({
      padID: id,
    }, (error, data) => {
      if (error) {
        new import_obsidian.Notice(`Error creating pad ${id}: ${error.message}`);
      } else {
        this.push_note_to_etherpad(id, text, note)
      }
    });
  }
  push_note_to_etherpad(id, text, note) {
    this.etherpad.setText({padID:id, text: text}, (error, data) => {
      if (error) {
        new import_obsidian.Notice(`向远端同步时遇到问题 ${id}: ${error.message}`);
      } else {
        this.update_db(note, id);
        new import_obsidian.Notice(`${id}已将本地最新内容上传远端`);
        this.show_logo();
        this.etherpad.getLastEdited({padID: id}, (error, timeData) => {
          if (error) {
            new import_obsidian.Notice(`获取远端最新版本时遇到问题 ${pad_id}: ${error.message}`);
          } else {
            this.update_db(note,id,timeData['lastEdited']);
          }
      })}
    });
  }
  auto_push_note_to_etherpad(id, text, note) {
    if (note == null)
        return;
      let etherpad_info = this.get_info(note);
      if (!etherpad_info)
    ...
    this.etherpad.getLastEdited({padID: id}, (error, timeData) => {
      if (error) {
        ...
        let remoteLastEdited = timeData['lastEdited'];
        let lastSyncTime = etherpad_info.etherpad_last_sync;
        console.log(`remoteLastEdited:${remoteLastEdited},lastSyncTime:${lastSyncTime}`);
        if (lastSyncTime >= remoteLastEdited) {
          this.push_note_to_etherpad(id, text);
        } else {
          new import_obsidian.Notice(`远端有新修改，未能同步。请手动同步内容！`);
        }
      }
    });
  }
...
var EtherpadSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Server port").setDesc("Server port").addText((text) => text.setPlaceholder("9001").setValue(this.plugin.settings.port).onChange((value) => __async(this, null, function* () {
    ...
    var list = ``;
    var db = this.plugin.db;
    for (const [key, value] of Object.entries(db)) {
      list+= `${key}\t${value.etherpad_id}\n`;
    }
    new import_obsidian.Setting(containerEl).setName("同步中文档").setDesc(list); 
  }
};
